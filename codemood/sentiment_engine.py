    ...
    def _analyze_ast_sentiment(self, tree: ast.AST) -> Tuple[float, List[str]]:
        ...
        class SentimentVisitor(ast.NodeVisitor):
            def visit_FunctionDef(self, node: ast.FunctionDef) -> None:
                ...
                if len(node.body) <= 10:
                    score += 0.3
                    reasoning.append("Contains concise functions")
                elif len(node.body) > 30:
                    score -= 0.5
                    reasoning.append("Contains very long functions")
                ...
                if (
                    ...
                    score += 0.4
                    reasoning.append("Functions have docstrings")
            ...
            def visit_ListComp(self, node: ast.ListComp) -> None:
                nonlocal score
                score += 0.6
                reasoning.append("Uses list comprehensions")
                self.generic_visit(node)

            def visit_With(self, node: ast.With) -> None:
                nonlocal score
                score += 0.5
                reasoning.append("Uses context managers")
                self.generic_visit(node)
